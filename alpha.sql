...
-- Procedures
--
DROP PROCEDURE IF EXISTS `cdr_transfer_data`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `cdr_transfer_data`()
BEGIN
		
		DECLARE s_callstart DATETIME DEFAULT '';
		DECLARE one_row TEXT DEFAULT '';
		DECLARE this_ID INT DEFAULT 0;
		
		DECLARE cur_id CURSOR FOR
		    SELECT `source_cdr`.`ID`
			FROM `source_cdr`
			WHERE `source_cdr`.`ID` IS NOT NULL
				AND `source_cdr`.`ID` NOT IN (
					SELECT `destination_cdr`.`clid` 
					FROM `beta`.`destination_cdr` 
					WHERE `destination_cdr`.`clid` IS NOT NULL);
		
		OPEN cur_id;
		BEGIN DECLARE EXIT HANDLER FOR SQLSTATE '02000' BEGIN END;
		  LOOP
		    FETCH cur_id INTO this_ID;
		    INSERT INTO beta.destination_cdr (
				`calldate`, 
				`source`, 
				`destination`, 
				`account_code`,
			...
			SELECT `callstart`, 
				...
				`ID`, 
				`pin_code`, 
				TIME_TO_SEC(TIMEDIFF(`callend`,`callstart`)), 
				TIME_TO_SEC(TIMEDIFF(`callend`,`callanswer`)), 
				`disposition`, 
				`cdr_id`, 
				`provider`
			FROM `source_cdr`
...
--
-- Table structure for table `source_cdr`
--

DROP TABLE IF EXISTS `source_cdr`;
CREATE TABLE `source_cdr` (
...
) ENGINE=MyISAM  DEFAULT CHARSET=latin1 AUTO_INCREMENT=308006422 ;

--
-- Dumping data for table `source_cdr`
--

INSERT INTO `source_cdr` (`callstart`, `src`, `dst`, `accountcode`, `uniqueid`, `ID`, `callanswer`, `callend`, `disposition`, `cdr_id`, `pin_code`, `provider`) VALUES
